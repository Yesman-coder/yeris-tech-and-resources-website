...
export function ContactView({ lang }: ContactViewProps) {
  ...
  if (submitted) {
    return (
      <div>
        ...
        <div className="t-green" style={{ fontSize: '14px', marginBottom: '8px' }}>
          {lang === 'en' ? '// transmission successful.' : '// transmisión exitosa.'}
        </div>
        <p className="t-gray" style={{ fontSize: '13px', lineHeight: '1.7' }}>
          {lang === 'en'
            ? "We'll be in touch within one business day. While you wait — type /game."
            : 'Nos pondremos en contacto en un día hábil. Mientras esperas — escribe /game.'}
        </p>
      </div>
    )
  }
  ...
  return (
    <div>
      <div className="t-section-title">
        {lang === 'en' ? 'OPEN A PROJECT CHANNEL' : 'ABRIR UN CANAL DE PROYECTO'}
      </div>
      ...
      >
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
          <div className="t-field">
            ...
            />
            {errors.name && (
              <span className="t-field-error">// {errors.name.message}</span>
            )}
          </div>
          <div className="t-field">
            <label className="t-label">
            ...
            />
            {errors.email && (
              <span className="t-field-error">// {errors.email.message}</span>
            )}
          </div>
        </div>
        ...
        <div className="t-field">
          ...
          />
          {errors.message && (
            <span className="t-field-error">// {errors.message.message}</span>
          )}
        </div>
